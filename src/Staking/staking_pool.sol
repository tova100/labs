...
contract Staking_pool {
    ...
    function whenDeposit(uint256 sum) public {
        ...
        User memory newUser = User({date_deposit: ((block.timestamp) / 68400) + 1, sum_deposit: sum});
    ...
    function withDraw(uint256 amount) public {
        ...
        require(
            userArray[0].date_deposit + 7 <= ((block.timestamp / 68400) + 1) && (userArray[0].sum_deposit > 0),
            "still not over 7 days"
        );
        uint256 i;
        for (
            i = 0;
            i < userArray.length && (s_amount < amount)
                && (userArray[i].date_deposit + 7 <= ((block.timestamp / 68400) + 1)) && (userArray[i].sum_deposit > 0);
            i++
        ) {
